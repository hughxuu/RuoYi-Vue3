...
const createDateLabels = (
  ...
  while (start <= end) {
    ...
    start.setDate(start.getDate() + (period === 'month' ? 7 : period === 'week' ? 2 : 1))
  }
...
const getJuvenileTrend = (params) => {
  const query = getQuery(params)
  const xAxis = createDateLabels(query.startDate, query.endDate, query.period).filter(
    (_, index) => index % 2 === 0
  )
  return {
    xAxis,
    self: [115, 175, 145, 210, 190, 145, 240, 210, 160, 150, 190, 230, 250, 200, 180, 220].slice(
      0,
      xAxis.length
    ),
    transfer: [78, 120, 95, 140, 125, 105, 160, 130, 120, 100, 145, 155, 180, 125, 130, 145].slice(
      0,
      xAxis.length
    ),
    dispatch: [45, 90, 60, 72, 85, 180, 100, 65, 80, 90, 110, 100, 125, 95, 140, 105].slice(
      0,
      xAxis.length
    ),
